...
{
	u32 temp;
	SysTick->LOAD = nus * fac_us;			  // 时间加载	  重装寄存器，决定倒计时的起始位
	SysTick->VAL = 0x00;					  // 清空计数器  当前值寄存器，当前倒计时的值
	SysTick->CTRL |= SysTick_CTRL_ENABLE_Msk; // 开始倒数	  控制和状态寄存器 控制开关、时钟源、中断等
	do
	{
		temp = SysTick->CTRL;
	} while ((temp & 0x01) && !(temp & (1 << 16))); // 等待时间到达    没倒计时到0，继续等
